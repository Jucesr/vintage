...
function show_catalog(catalog_id) {
  ...
  //package_item items.
  var i = 1;
  catalogs_data[catalog_id].packages.map(function (package_item) {
    var package_div = $('<div class="package">');
    var package_name = $('<h4>' + package_item.name + '</h4>');
    ...
    package_item.info.map(function (info) {
  ...
  //This part is for adjust the height of the package_item information base on how many items it has
  var max_height = 0;
...
function reserve(params) {
  ...
  var package_item = catalogs_data[index[0]].packages[index[1] - 1];
  var text = 'Hola \n Me gustaria contratar el paquete \'' + package_item.name + '\' de \'' + catalog + '\'. Para mi proximo evento. \n La informacion del paquete es: \n';
  var info = '';
  package_item.info.map(function (i) {
    info += ' \n' + i;
  });
  text += info;
...
$('#contact_form').submit(function (event) {
  ...
  if (error) {
    ...
    $.ajax({
      ...
      complete: function complete(xhr, ajaxOptions, thrownError) {
        if (xhr.status == 200) {
